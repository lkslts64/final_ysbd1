...
int main(int argc,char** argv)
{
	...
	*/
	...
	C1: Create the  index.
	*/
	fprintf(file,"@Checkpoint 1: Create Index\n");
	int createNumCode=HT_CreateIndex(fileName,attrType,attrName,attrLength,buckets);
	if (createNumCode==0)
	{
		fprintf(file,"Checkpoint Result 1: SUCCESS\n");
	}
	else
	{
		fprintf(file,"Checkpoint Result 1: FAIL\n");
	}
	HT_info* hi;
	/*
	C2: Open index.
	*/
	fprintf(file,"@Checkpoint 2: Open Index\n");
	hi=HT_OpenIndex(fileName);
	if(hi!=NULL && hi->attrType==attrType && strcmp(hi->attrName,attrName)==0)
	{
		fprintf(file,"Checkpoint Result 2: SUCCESS\n");
	}
	else
	{
		fprintf(file,"Checkpoint Result 2: FAIL\n");
	}
	/*
	C3: Insert records.
	*/
	fprintf(file,"@Checkpoint 3: Insert Records\n");
	...
	C4: Get all entries.
	*/
	fprintf(file,"@Checkpoint 4: Get all entries (Expecting greater than zero return code)\n");
	int ch4=0;
	for (int i=0;i<testRecordsNumber;i++)
	{
		Record record;
	...
	if(ch4>0)
	{
		fprintf(file,"Checkpoint Result 4: FAIL\n");
	}
	else
	{
		fprintf(file,"Checkpoint Result 4: SUCCESS\n");
	}
	fprintf(file,"@Checkpoint 5: Get all entries (Expecting -1 return code, records do not exist)\n");
	int ch5=0;
	/*
	...
	*/
	...
	{
		fprintf(file,"Checkpoint Result 5: FAIL\n");
	}
	else
	{
		fprintf(file,"Checkpoint Result 5: SUCCESS\n");
	}
	/*
	C6: Delete entries.
	*/
	fprintf(file,"@Checkpoint 6: Delete some entries\n");
	int ch6=0;
	...
	if(ch6!=0)
	{
		fprintf(file,"Checkpoint Result 6: FAIL\n");
	}
	else
	{
		fprintf(file,"Checkpoint Result 6: SUCCESS\n");
	}
	/*
	C7: Get all entries.
	*/
	fprintf(file,"@Checkpoint 7: Get all entries (%d should not exist)\n",testDeleteRecords);
	int ch7=0;
	for (int i=0;i<testRecordsNumber;i++)
	{
		Record record;
	...
	if(ch7!=deletesTest)
	{
		fprintf(file,"Checkpoint Result 7: FAIL\n");
	}
	else
	{
		fprintf(file,"Checkpoint Result 7: SUCCESS\n");
	}
	/*
	...
	*/
	fprintf(file,"@Checkpoint 8: Create Secondary/ Open Index\n");
	int createErrorCode=SHT_CreateSecondaryIndex(sfileName,sAttrName,sAttrLength,sBuckets,fileName);
	if (createErrorCode<0)
	{
		fprintf(file,"Checkpoint Result 8: FAILED\n");
		return -1;
	}
	SHT_info* shi=SHT_OpenSecondaryIndex(sfileName);
	if(shi!=NULL)
	{
		fprintf(file,"Checkpoint Result 8 SUCCESS\n");
	}
	else
	{
		fprintf(file,"Checkpoint Result 8: FAIL\n");
	}
	/*
	...
	*/
	fprintf(file,"@Checkpoint 9: Insert Records Secondary\n");
	int ch9=0;
	for (int i=testRecordsNumber;i<testRecordsNumber*2;i++)
	{
		Record record;
	...
	if (ch9==0)
	{
		fprintf(file,"Checkpoint Result 9: SUCCESS\n");
	}
	else
	{
		fprintf(file,"Checkpoint Result 9: Fail\n");
	}
	/*
	C10: Get all entries using secondary the  index.
	*/
	fprintf(file,"@Checkpoint 10: SHT Get all entries (All should exist except the deleted)\n");
	int ch10=0;
	...
	if(ch10>deletesTest)
	{
		...
		*/
		printf("ch10 = %d and deletesTest = %d\n",ch10,deletesTest);
		fprintf(file,"Checkpoint Result 10: FAIL\n");
	}
	else
	{
		fprintf(file,"Checkpoint Result 10: SUCCESS\n");
	}
	fprintf(file,"@Checkpoint 11\n");
	int htCloseError=HT_CloseIndex(hi);
	int shtCloseError=SHT_CloseSecondaryIndex(shi);
	if (htCloseError==0 && shtCloseError==0)
	{
		fprintf(file,"Checkpoint Result 11: SUCCESS\n");
	}
	else
	{
		fprintf(file,"Checkpoint Result 11: FAIL\n");
	}
	/*
	Hash statistics.
	*/
